...
class OrderAdapter : RecyclerView.Adapter<OrderAdapter.OrderViewHolder>() {
    ...
    fun submitList(newOrders: List<Order>) {
        orders.clear()
        // Filter orders to show only those created by or accepted by the current user
        orders.addAll(newOrders.filter { it.userId == currentUserId || it.acceptedBy == currentUserId })
        notifyDataSetChanged()
    }
    ...
    inner class OrderViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(order: Order) {
            ...
            quantityTxt.text = "Quantity: ${order.quantity}"
            priceTxt.text = "BDT ${order.totalPrice}"
            rewardTxt.text = "Reward: ${(order.totalPrice * 0.1).toInt()} BDT"
            timeTxt.text = order.time
            ...
            if (order.userId == currentUserId) {
                ...
                cancelButton.visibility = View.VISIBLE
                acceptedSticker.visibility = View.GONE
            } else if (order.acceptedBy == currentUserId) {
                // Order accepted by the current user
                acceptButton.visibility = View.GONE
                cancelButton.visibility = View.VISIBLE
                acceptedSticker.visibility = View.VISIBLE
            } else if (order.acceptedBy.isNotEmpty()) {
                // Order accepted by someone else (hide from other users)
